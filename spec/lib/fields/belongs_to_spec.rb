...
describe Administrate::Field::BelongsTo do
  ...
  describe "#selected_option" do
    it "reads the correct value through complex associations" do
      country = create(:country)
      customer = create(:customer, territory: country)

      field = Administrate::Field::BelongsTo.new(:territory, country, :show, resource: customer)

      expect(field.selected_option).to eq(country.code)
    end
  end

  describe "#associated_resource_options" do
    it "reads the correct values through complex associations" do
      countries = create_list(:country, 5)
      customer = create(:customer, territory: countries.first)

      field = Administrate::Field::BelongsTo.new(:territory, customer, :show, resource: customer)

      actual_resource_ids = field.associated_resource_options.compact.to_h.values
      expected_resource_ids = countries.pluck(:code).sort
      expect(actual_resource_ids.sort).to eq(expected_resource_ids)
    end
